...
class PlayState extends MusicBeatState
{
	...
	//NORMAL KITE STUFF
	var cityk:FlxSprite;
	var lightsk:FlxSprite;
	var streetk:FlxSprite;
	...
	var stars:FlxSprite;
	var stars2:FlxSprite;
	var coinsBack:FlxSprite;
	var coinsBack2:FlxSprite;
	var coinsFront:FlxSprite;
	var coinsFront2:FlxSprite;
	...
	override public function create()
	{
		...
		switch(songLowercase)
		{
			...
			case 'calm' | 'dominating' | 'skills' | 'envy' | 'calm-classic' | 'dominating-classic' | 'skills-classic' | 'envy-classic':
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				case 'kitebg':
					{
						defaultCamZoom = 0.7;
						curStage = 'kitebg';
						cityk = new FlxSprite(-800, -600);
						cityk.frames = Paths.getSparrowAtlas('kitebg/fondokite', 'shared');
						cityk.animation.addByPrefix('idle', 'halloweem bg');
						cityk.animation.play('idle');
						cityk.antialiasing = true;
						//city.scrollFactor.set(0.1, 0.1);
						add(cityk);

						lightsk = new FlxSprite(-420, -198);
						lightsk.frames = Paths.getSparrowAtlas('kitebg/luces', 'shared');
						lightsk.animation.addByPrefix('idle', 'halloweem bg');
						lightsk.animation.play('idle');
						lightsk.antialiasing = true;
						//lights.scrollFactor.set(0.1, 0.1);
						add(lightsk);

						streetk = new FlxSprite(-900, 450);
						streetk.frames = Paths.getSparrowAtlas('kitebg/piso', 'shared');
						streetk.animation.addByPrefix('idle', 'Símbolo 9');
						streetk.animation.play('idle');
						streetk.antialiasing = true;
						//street.scrollFactor.set(0.3, 0.3);
						add(streetk);
					}
					case 'sky':
					{
						...
						var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image(curStage + '/bg'));
						...
						stars = new FlxSprite(0, 0).loadGraphic(Paths.image(curStage + '/stars'));
						...
						stars2 = new FlxSprite(stars.width, 0).loadGraphic(Paths.image(curStage + '/stars'));
						...
						coinsBack = new FlxSprite(0, 0).loadGraphic(Paths.image(curStage + '/coins1'), true, 160, 80);
						...
						coinsBack2 = new FlxSprite(coinsBack.width, 0).loadGraphic(Paths.image(curStage + '/coins1'), true, 160, 80);
						...
						coinsFront = new FlxSprite(0, 0).loadGraphic(Paths.image(curStage + '/coins2'), true, 160, 81);
						...
						coinsFront2 = new FlxSprite(coinsFront.width, 0).loadGraphic(Paths.image(curStage + '/coins2'), true, 160, 81);
						...
						var platform:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image(curStage + '/clouds'));
				...
				case 'kitebg-classic':
					{
						defaultCamZoom = 0.9;
						curStage = 'kitebg-classic';
						var bg:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('funkytownport'));
						bg.scale.set(1.5, 1.7);
		...
		switch (SONG.player2)
		{
			...
			case 'dad':
				camPos.x += 400;
			case 'kite' | 'kite-skills' | 'kite-envy':
				dad.x -= 300;
				dad.y -= 20;
			case 'kiteold':
				dad.y += 170;
			case 'kite-bit':
				camPos.set(dad.getMidpoint().x + 180, dad.getMidpoint().y - 9);
				dad.x += 70;
				dad.y += 24;
		}
		...
		var doofOLD:DialogueBoxOLD = new DialogueBoxOLD(false, dialogue);
		// doof.x += 70;
		// doof.y = FlxG.height * 0.5;
		doofOLD.scrollFactor.set();
		doofOLD.finishThing = startCountdown;
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
		{
			...
			if (PlayStateChangeables.useDownscroll)
				songName.y -= 3;
			if(curStage == 'sky')
				songName.setFormat(Paths.font("pixel.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			else
				songName.setFormat(Paths.font("JandaManateeSolid.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(curStage == 'sky')
			kadeEngineWatermark.setFormat(Paths.font("pixel.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		else
			kadeEngineWatermark.setFormat(Paths.font("JandaManateeSolid.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(curStage == 'sky')
			...
			scoreTxt.setFormat(Paths.font("JandaManateeSolid.ttf"), 16, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		replayTxt = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0, "REPLAY",
			20);
		if(curStage == 'sky')
			replayTxt.setFormat(Paths.font("pixel.otf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		else
			replayTxt.setFormat(Paths.font("JandaManateeSolid.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botPlayState = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0,
			"BOTPLAY", 20);
		if(curStage == 'sky')
			botPlayState.setFormat(Paths.font("pixel.otf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		else
			botPlayState.setFormat(Paths.font("JandaManateeSolid.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'kite-bit':
						...
						camFollow.y = dad.getMidpoint().y - 9;
					case 'kiteold':
	...
	override function stepHit()
	{
		...
		if (FlxG.save.data.distractions)
		{
			if (SONG.song.toLowerCase() == 'dominating')
				{
					switch(curStep)
					{
						case 580 | 586 | 591 | 612 | 618 | 623:
							camGame.flash(FlxColor.WHITE, .3);
						case 703:
							camGame.flash(FlxColor.WHITE, .7);
							FlxTween.tween(cityk, {alpha: 0}, 0.5);
							FlxTween.tween(streetk, {alpha: 0}, 0.5);
						case 815: //idk wtf is this
						...
						case 831:
							FlxG.camera.zoom = defaultCamZoom;
							FlxTween.tween(cityk, {alpha: 1}, 0.1);
							FlxTween.tween(streetk, {alpha: 1}, 0.1);
							camGame.flash(FlxColor.WHITE, .7);
					}
				}
			else if (SONG.song.toLowerCase() == 'skills')
				{
					if (curStep == 256)
					{
						for (i in 0...strumLineNotes.length)
						{
							if (i <=3)
							{
								FlxTween.tween(strumLineNotes.members[i], {x: strumLineNotes.members[i].x + 620}, 0.3);
							}
							else if (i > 3 && i <= 7)
							{
								FlxTween.tween(strumLineNotes.members[i], {x: strumLineNotes.members[i].x - 620}, 0.3);
					...
					else if (curStep == 510)
					{
					...
					else if (curStep == 895)
						{
							for (i in 0...strumLineNotes.length)
							{
								...
								{
									if(PlayStateChangeables.useDownscroll)
										FlxTween.tween(strumLineNotes.members[i], {y: strumLineNotes.members[i].y - 200}, 0.3);	
									else
										FlxTween.tween(strumLineNotes.members[i], {y: strumLineNotes.members[i].y + 200}, 0.3);	
								}
							}
							FlxTween.tween(cityk, {alpha: 0}, 0.1);
							FlxTween.tween(streetk, {alpha: 0}, 0.1);
							FlxTween.tween(lightsk, {alpha: 0}, 0.1);
							camGame.flash(FlxColor.WHITE, .7);
						}
					else if(curStep == 927)
					{
					...
					else if(curStep == 992)
					{
					...
					else if(curStep == 1024)
						{
							...
							FlxTween.tween(lightsk, {alpha: 1}, 0.1);
							camGame.flash(FlxColor.WHITE, .7);
						}
					else if(curStep == 1247)
					{
						FlxTween.tween(FlxG.camera, {zoom: 1.6}, 2.3, {
							ease: FlxEase.expoOut
						});
					}
					else if(curStep == 1275)
					{
						remove(dad);
						dad = new Character(dad.x, dad.y, 'kite-skills');
						add(dad);
					}
			...
			else if (SONG.song.toLowerCase() == 'envy')
				{
					switch(curStep)
					{
						case 511 | 576 | 607 | 619 | 623:
							camGame.flash(FlxColor.WHITE, .7);
							FlxG.camera.zoom = (defaultCamZoom * 1.3);
						case 615:
							camGame.flash(FlxColor.WHITE, .7);
						...
						case 687:
							FlxTween.tween(cityk, {alpha: 0}, 1.5);
							FlxTween.tween(streetk, {alpha: 0}, 1.5);
						case 735 | 740 | 744 | 748 | 1027 | 1030 | 1059 | 1061:
							FlxG.camera.zoom = (defaultCamZoom * 1.4);
						case 1015:
							FlxTween.tween(cityk, {alpha: 1}, 1);
							FlxTween.tween(streetk, {alpha: 1}, 1);
							healthBarBG.visible = true;
							kadeEngineWatermark.visible = true;
							healthBar.visible = true;
							iconP1.visible = true;
						...
						case 1151:
							FlxTween.tween(FlxG.camera, {zoom: 1.2}, 11, {
								ease: FlxEase.expoOut
							});
							camGame.flash(FlxColor.WHITE, .7);
							FlxTween.tween(cityk, {alpha: 0}, 2);
							FlxTween.tween(streetk, {alpha: 0}, 2);
							camHUD.visible = false;
						case 1275:
							camHUD.visible = true;
						case 1279:
						...
						case 1367:
							FlxTween.tween(cityk, {alpha: 1}, 2);
							FlxTween.tween(streetk, {alpha: 1}, 2);
					}
				}
				else if (SONG.song.toLowerCase() == 'dominating classic')
					{
						if (curStep == 685)
							{
						...
						if (curStep == 703)
							{
								FlxG.camera.fade(FlxColor.WHITE, .7, true);
							}
						if (curStep == 831)
							{
								FlxG.camera.fade(FlxColor.WHITE, .7, true);
				...
				else if (SONG.song.toLowerCase() == 'envy classic')
					{
						if (curStep == 1273)
							{
								FlxG.camera.fade(FlxColor.WHITE, .7, true);
							}
						if (curStep >= 1823 && curStep < 2038)
							{
	...
	override function beatHit()
	{
		...
		if (FlxG.save.data.distractions)
		{
			if (SONG.song.toLowerCase() == 'envy')
			{
				if(curBeat >= 368 && curBeat % 2 == 0 && curBeat <= 512)
				{
					camGame.flash(FlxColor.WHITE, .7);
				}
			}
