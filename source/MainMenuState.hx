...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		magenta = new FlxBackdrop(Paths.image('cuadrakite'), 0, 0, true, true); // backgrounds are the only hardcoded thing sorry :(
		magenta.setPosition(0, 0);
		magenta.antialiasing = true;
		magenta.scrollFactor.set();
		magenta.velocity.set(20, 0);
		magenta.visible = false;
		magenta.antialiasing = FlxG.save.data.antialiasing;
		magenta.color = 0xFF616d79;
		add(magenta);
		// magenta.scrollFactor.set();
		...
		for (i in 0...optionShit.length)
		{
			...
			if (firstStart)
				FlxTween.tween(menuItem,{y: 60 + (i * 160), x: 60 + (i * 160)},1 + (i * 0.25) ,{ease: FlxEase.expoInOut, onComplete: function(flxTween:FlxTween) 
					{ 
